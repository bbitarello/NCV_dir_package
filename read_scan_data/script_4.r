...
setDT(LWK.2) #do I still need this?
...
AWS.2<-All.Res3[[1]]
mclpply(bin.vec1, function(x) (which(AWS.2$Nr.IS==x)))->temp.AWS
...
########################################################################################################################################
########################################################################################################################################
 st(mclapply(temp2.CEU, function(x) (sum(CEU.2$NCDf5[x]>=l.bin.vec2.eu[[1]]$ncvFD_f0.5)/nsims)))->CEU.2$P.val.NCDf0.5[temp2.CEU]
