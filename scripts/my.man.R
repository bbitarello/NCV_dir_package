my.manhattan <- function(x, chr="CHR", bp="BP", p="P", snp="SNP", 
    ...
    if (nchr==1) {
        with(d, points(pos, logp, pch=20, col=col[1], cex=0.7,...))
    } else {
    ...
    if (suggestiveline) abline(h=suggestiveline, col="orange1", lty=2)
    if (genomewideline) abline(h=genomewideline, col="orangered3", lty=2)
   ...
   # Highlight snps from a character vector
    if (!is.null(highlight2)) {
        ...
        with(d.highlight2, points(pos, logp, col="cadetblue", pch=20, cex=0.85, ...))
    }
    ...
    if (!is.null(highlight)) {
        ...
        with(d.highlight, points(pos, logp, col="violetred1", pch=20, cex=0.90, ...)) 
    }
