library(data.table)
library(dplyr)
load("/mnt/expressions/miguel/gwas_expr/expr.rdat")
fread('/mnt/sequencedb/PopGen/barbara/NCV_dir_package/read_scan_data/bedfiles/all_scanned_genes.txt', header=F)-> scanned

fread('/mnt/sequencedb/PopGen/barbara/NCV_dir_package/read_scan_data/bedfiles/extreme.genes.bed')-> extreme_genes
fread('/mnt/sequencedb/PopGen/barbara/NCV_dir_package/read_scan_data/bedfiles/cand.extreme.genes.bed')-> cand_extreme_genes
...
##let's start
#completely arbitrary cutofss...these measure ar enot normalized by gene length, byt only by all idnividuals. So some genes have huge values because they are larger.
...
for(i in 1:53){
cbind(ex.t.gc.dt.scanned[[i]], data.frame(RS0=rowSums(ex.t.gc.dt.scanned[[i]][,-1]>0), RS1=rowSums(ex.t.gc.dt.scanned[[i]][,-1]>1), RS10=rowSums(ex.t.gc.dt.scanned[[i]][,-1]>10), RS100=rowSums(ex.t.gc.dt.scanned[[i]][,-1]>100), RS500=rowSums(ex.t.gc.dt.scanned[[i]][,-1]>500))) %>% as.data.table %>% mutate(Expr_Status=ifelse(RS100>5, "TRUE", "FALSE")) %>% as.data.table -> ex.t.gc.dt.scanned[[i]]
}
