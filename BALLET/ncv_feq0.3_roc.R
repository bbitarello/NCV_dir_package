...
## 2. Make ROC curves
#modified by BBitarello on 19.1.2016

source('/mnt/sequencedb/PopGen/barbara/SCRIPTS/ms_tools.R')
setwd("/mnt/sequencedb/PopGen/barbara/simulations/clone_cee_sims/")



ncv <- function(x,fr.eq=0.3, fd=TRUE) {
...
p2d <- function(x) { # calc2ulate Polymorphism-to-Divergence (p2d) as number of SNPs divided by number of fixed differences. 
  ## x: tha data as in ms format
  x[x == "2"] <- "1" # in case of triallelic sites
  x1 <- x[-nrow(x),]
  daf <- apply(x1,2,function(y) sum(as.numeric(y)))/nrow(x1) # the derived allele frequency
...
for (n in 1:length(popN)) {
  ...
  for(j in 1:length(Tbs)) {
    ...
    for(i in 1:length(basepairs)) {
      ...
      ## NCV with FDs
      ncvFD.n <- mclapply(ms.n, function(x) sapply(pop.id, function(y) ncv(x[c(y,nrow(x)),],fd=TRUE,fr.eq=0.4)),mc.cores=detectCores()); ncvFD.n <-lapply(1:3,function(x) unlist(lapply(ncvFD.n,function(y) y[x])))
      ## NCV without FDs
      ncv.n <- mclapply(ms.n, function(x) sapply(pop.id, function(y) ncv(x[y,],fd=FALSE, fr.eq=0.4)),mc.cores=detectCores()); ncv.n <- lapply(1:3,function(x) unlist(lapply(ncv.n,function(y) y[x])))
      ## HKA
      ...
      write.table(round(as.numeric(out),4),sub("msstats","ncv+hka.out",o),row.names=F,quote=F,sep="\t")
      ...
      for(f in 1:length(fr.eq)) {
        ...
        ## NCV with FDs
        ncvFD.s <- mclapply(ms.s, function(x) sapply(pop.id, function(y) ncv(x[c(y,nrow(x)),],fd=TRUE, fr.eq=0.4)),mc.cores=detectCores())
        ncvFD.s <- lapply(1:3,function(x) unlist(lapply(ncvFD.s,function(y) y[x])))
        ## NCV without FDs
        ncv.s <- mclapply(ms.s, function(x) sapply(pop.id, function(y) ncv(x[y,],, fr.eq=0.4, fd=FALSE)),mc.cores=detectCores())
        ...
        out <- cbind(do.call(cbind,ncvFD.s), do.call(cbind,ncv.s),do.call(cbind,hka.s)); colnames(out) <- c("ncvFDfeq0.4_AFR","ncvFDfeq0.4_EUR","ncvFDfeq0.4_ASN","ncv_feq0.4AFR","ncvfeq0.4_EUR","ncvfeq0.4_ASN","hka_AFR","hka_EUR","hka_ASN")  
        write.table(round(as.numeric(out),4),sub("msstats","ncv+hka.out",o),row.names=F,quote=F,sep="\t")
...
### Concatenating as a list the ROC curves for different tests
        res <- list(ncvFD.ROC, ncv.ROC, taj.ROC,hka.ROC,ncv_hka.ROC,taj_hka.ROC); names(res) <- c("ncvFDfeq0.4.ROC", "ncvfeq0.4.ROC", "taj.ROC", "hka.ROC", "ncv_hka.ROC","taj_hka.ROC")
  ...
  save.image("Results.all.ROC1.NCVfeq0.4.RData"); cat("N chrom",popN[n],"done\n")
}
